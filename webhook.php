...
if (isset($_GET['notify']) && $_GET['notify'] == 'pixup') {
	...
	if ($status == 'PAID') {
		...
		$product_list = $conn->query('SELECT pending_numbers, paid_numbers, qty_numbers
			FROM product_list
			WHERE id = \'' . $product_id . '\'');
		...
		if ($status_order == '1') {
			...
			$sql_ol = 'UPDATE order_list SET status = \'2\', date_updated = \'' . $payment_date . '\', whatsapp_status = \'\' WHERE id = \'' . intval($pedido_id) . '\'';
			$conn->query($sql_ol);
...
if (isset($_GET['notify']) == 'ondapay') {
	...
	$status = $event['status'] ?? NULL;
	$pedido_id = $event['transaction_id'] ?? NULL;
	$eventName = $event['type_transaction'] ?? NULL;
	$product_id = '';
	$ref = '';

	if ($eventName == 'CASH_IN') {
		$qry = $conn->query('SELECT o.status, o.product_id, o.total_amount, o.quantity, c.firstname, c.lastname, c.phone, o.referral_id' . "\r\n\t\t\t" . 'FROM order_list o' . "\r\n\t\t\t" . 'INNER JOIN customer_list c ON o.customer_id = c.id' . "\r\n\t\t\t" . 'WHERE o.id_mp = \'' . $pedido_id . '\'');
		...
		if ($status == 'PAID_OUT') {
			if ($status_order == '1') {
				date_default_timezone_set('America/Sao_Paulo');
				$payment_date = date('Y-m-d H:i:s');
				$sql_ol = 'UPDATE order_list SET status = \'2\', date_updated = \'' . $payment_date . '\', whatsapp_status = \'\' WHERE id_mp = \'' . $pedido_id . '\'';
				$conn->query($sql_ol);
				$sql_pl = 'UPDATE product_list SET pending_numbers = \'' . $updatePending . '\', paid_numbers = \'' . $updatePaid . '\' WHERE id = \'' . $product_id . '\'';
				$conn->query($sql_pl);
