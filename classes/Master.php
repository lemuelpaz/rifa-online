...
class Master extends DBConnection
{
	...
	public function deactive_license()
	{
		...
		if (!empty($license)) {
			$update = $this->conn->query('UPDATE system_info SET meta_value = \'\' WHERE meta_field = \'carlos\'');
	...
	public function place_order()
	{
		...
		if ($insert) {
			...
			if (($this->settings->info('ativopay') == 1) && 0 < $total_amount) {
			...
			if (($this->settings->info('bestfy') == 1) && 0 < $total_amount) {
