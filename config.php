...
function ondapay_pix($order_id, $total_amount, $customer_name, $client_email, $order_expiration, $customer_phone, $cpf)
{
    ...
    $data = array(
        ...
        "payer" => array(
            ...
            "email" => $client_email // Email Do Pagador
        ),
    ...
    //var_dump($responseQR);
    curl_close($curlQR);
    $decodeContentQR = json_decode($responseQR);

    try {
        if (isset($decodeContentQR->qrcode)) {
            $pix_code = $decodeContentQR->qrcode;
            $txID = $decodeContentQR->id_transaction;
            $pix_qrcode = $decodeContentQR->qrcode_base64;
            ...
            $sql = "UPDATE order_list
                    SET payment_method = 'OndaPay', pix_code = '{$pix_code}', pix_qrcode = '{$pix_qrcode}', order_expiration = '{$order_expiration}', txid = '{$txID}'
                    WHERE id = {$order_id}";
        ...
        echo "Erro na atualização: " . $decodeContentQR->errors->errorMessage;
    }
}
...
function ativopay_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration)
{
	global $conn;
	global $_settings;
...
function bestfy_generate_pix($order_id, $amount, $client_name, $order_expiration, $customer_cpf)
{
	global $conn;
	global $_settings;
	...
	$order_user = $client_name;
	$client_key = $_settings->info('bestfy_client_id');
	$client_secret = $_settings->info('bestfy_client_secret');
...
function page_view($origin, $page, $product_id = null, $user_id = null)
{
    ...
    $sql = "INSERT INTO `page_view`(`product_id`, `customer_id`, `page`, `origin`) VALUES ('$product_id','$user_id','$page','$origin')";
    $result = $conn->query($sql);
