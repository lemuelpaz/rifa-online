...
class Main extends DBConnection
{
            ...
            if (0 < $user_verify->num_rows) {
                $update = $this->conn->query(
                    "UPDATE referral SET percentage = " .
                        ...
                        $status .
                        ", amount_paid = " .
                        (isset($_POST['amount_paid']) ? floatval($_POST['amount_paid']) : 0) .
                        ' WHERE customer_id = \'' .
                        $user_id .
                        '\''
                );
                // Atualizar is_affiliate conforme status
                $this->conn->query("UPDATE customer_list SET is_affiliate = $status WHERE id = '$user_id'");
                if ($update) {
                    $resp["status"] = "success";
                ...
                $insert = $this->conn->query(
                    "INSERT INTO referral (status, referral_code, percentage, amount_paid, amount_pending, customer_id) VALUES (" .
                        $status . ", " . $user_id . ", " . $percentage . ", 0, 0, " . $user_id . ")"
                );
                if ($insert) {
                    $this->conn->query("UPDATE customer_list SET is_affiliate = $status WHERE id = '$user_id'");
                    $resp["status"] = "success";
    ...
    public function place_order()
    {
            ...
            if ($this->settings->info("ondapay") == 1) {
            ...
            $query =
                'SELECT discount_qty, enable_discount, enable_double, double_ini, double_fim, discount_amount,discount_roleta,roleta_qty, roleta_amount, box_qty, box_amount, enable_cumulative_discount, enable_sale, sale_qty, roleta, box, sale_price, status, qty_numbers, probabilidade, pending_numbers, paid_numbers, date_of_draw, habilitar_cota_sorte, cota_sorte_ini, cota_sorte_fim, cota_sorte, quantidade_compra_sorte FROM product_list WHERE id = \'' .
                $product_id .
                '\'';
            ...
            if ($roletas) {
                foreach ($roletas as $key => $roleta) {
                    $qtdRoleta = 1;
                    // Se for o último item ou o valor de $quantity for entre o valor atual e o próximo
                    if ($key + 1 < count($roletas)) {
                        $nextRoleta = $roletas[$key + 1];
                        // Verifica se o valor de $quantity está entre os valores de 'amount' (inclusive)
                        if ($quantity >= $roleta['amount'] && $quantity < $nextRoleta['amount']) {
                            $qtdRoleta = $roleta['qty'];
                            break;
                        }
                    } else {
                        // Para o último intervalo, incluindo o limite superior
            ...
            $qtdBox = 0;  // Inicializando como 0

            if ($boxs) {
                foreach ($boxs as $key => $box) {
                    // Verifica se o valor de $quantity está entre o valor atual e o próximo
                    if ($key + 1 < count($boxs)) {
                        $nextBox = $boxs[$key + 1];
                        // Verifica se o valor de $quantity está dentro do intervalo atual
                        if ($quantity >= $box['amount'] && $quantity < $nextBox['amount']) {
                            $qtdBox = $box['qty'];  // Atualiza o valor de $qtdBox
                            break;  // Sai do loop assim que encontrar a caixa correspondente
                        }
                    } else {
                        // Para o último intervalo, se não houver mais caixas depois
                        if ($quantity >= $box['amount']) {
                            $qtdBox = $box['qty'];  // Atualiza o valor de $qtdBox
                            break;  // Sai do loop ao encontrar a caixa correspondente
                        }
                    }
                }
            } else {
                $qtdBox = 1;  // Se não houver caixas, define o valor de $qtdBox como 1
            }
            ...
            if ($insert) {
                ...
                if ($use_manual_numbers) {
                    ...
                    while (count($numeris) < $total_numbers_generated) {
                        // Geração do número aleatório normal
                ...
                if (($this->settings->info('ondapay') == 1) &&  $total_amount > 0) {
					ondapay_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration, $customer_phone, $customer_cpf);
				}
				if (($this->settings->info('pixup') == 1) &&  $total_amount > 0) {
					pixup_generate_pix($oid, $total_amount, $customer_name, $customer_cpf, $order_expiration);
				}
...
");
    ...
    public function search_raffle_smallest_and_largest_number()
    {
        ...
        if ($valorMinimo) {
            $valorMinimo = str_replace(",", ".", $valorMinimo);
            $where .= ' AND total_amount >= "' . $valorMinimo . '"';
        }
            ...
            foreach ($cotas_premiadas_sold as $cota) {
                ...
                if ($cota != '' && !$minor && !$major) {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        echo '    <div class="app-titulos-premiados--item bg-gradient-blue2 app-titulos-premiados--selected">';
                        echo '        <div class="app-titulos-premiados--premiacao"><span class="btn p-0 btn-primary btn-sm" style="min-width:100px;background: #fff;color:#000;">' . $cota . '</span></div>';
                        echo '        <div class="app-titulos-premiados--premiacao">' . $prize . '</div>';
            ...
            foreach ($cotas_premiadas_available as $cota) {
                ...
                if ($cota != '' && !$minor && !$major) {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        ...
                        echo '    <div class="app-titulos-premiados--item bg-white p-2">';
                        echo '        <div class="app-titulos-premiados--premiacao"><span class="btn p-0 btn-light btn-sm rounded" style="min-width:100px;background: #919EAB;color:#000;">' . $cota . '</span></div>';
                        echo '        <div class="app-titulos-premiados--premiacao">' . $prize . '</div>';
                        echo '        <div class="app-titulos-premiados--ganhador"><span>Disponível</span></div>';
                        echo '    </div>';
                    } else {
                        //  echo '<div class=" rounded p-1 w-100 d-flex justify-content-center align-items-center">';
                        //  echo '    <div class="numero rounded-pill d-flex justify-content-center align-items-center btn text-start '.$btnTheme.' btn-sm p-0 text-nowrap font-xss">';
        ...
        if ($cotas_premiadas_sold) {
            foreach ($cotas_premiadas_sold as $cota) {
                ...
                if ($cota != '' && !$minor && !$major) {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        echo '    <div class="app-titulos-premiados--item bg-gradient-blue2 app-titulos-premiados--selected">';
                        echo '        <div class="wd-6 new_gradient_anime py-1 --md Premiada btn btn-sm btn-light text-dark" style="min-width:75px !important; font-weight:500; margin-right:4px; font-size:14px">' . $prize . '</div>';
                        echo '        <div class="app-titulos-premiados--ganhador"><span>' . $customer_name . '';
                        echo '        <i class="bi bi-trophy-fill text-warning"></i></span></div>';
                        echo '    </div>';
                    } else {
            ...
            foreach ($cotas_premiadas_available as $cota) {
                ...
                if ($cota != '' && !$minor && !$major) {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        ...
                        echo '    <div class="app-titulos-premiados--item bg-white p-2">';
                        echo '        <div class="wd-6 new_gradient_anime py-1 --md Premiada btn btn-sm bg-gradient-blue3 text-dark" style="min-width:75px !important; font-weight:500; margin-right:4px; font-size:14px">' . $prize . '</div>';
                        echo '        <div class="app-titulos-premiados--ganhador"><span>Disponível</span></div>';
                        echo '    </div>';
                    } else {

                        echo '    <div class="app-titulos-premiados--item bg-dark p-2">';
                        echo '        <div class="wd-6 new_gradient_anime --md Premiada btn btn-sm btn-light text-dark" style="min-width:75px !important; font-weight:500; margin-right:4px; font-size:14px">' . $prize . '</div>';
                        echo '        <div class="app-titulos-premiados--ganhador"><span>Disponível</span></div>';
                        echo '    </div>';
                    }
                }
            ...
            foreach ($cotas_premiadas_available as $cota) {
                ...
                if ($cota != '' && !$minor && !$major) {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        ...
                        echo '    <div class="app-titulos-premiados--item bg-white p-2">';
                        echo '        <div class="wd-6 new_gradient_anime --md Premiada btn btn-sm bg-gradient-blue3 text-dark" style="min-width:75px !important; font-weight:500; margin-right:4px; font-size:14px">' . $prize . '</div>';
                        echo '        <div class="app-titulos-premiados--ganhador"><span>Disponível</span></div>';
                        echo '    </div>';
                    } else {
