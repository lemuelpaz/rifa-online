...
$order_numbers = implode(",", $numeris) . ",";
		 ...
		 mercadopago_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration);
				...
				if (($this->settings->info('ativopay') == 1) && 0 < $total_amount) {
					ativopay_generate_pix($oid, $total_amount, $customer_name, $customer_cpf, $order_expiration);
                    $resp['gateway'] = 'ativopay';
				}
				...
				if (($this->settings->info('bestfy') == 1) && 0 < $total_amount) {
					bestfy_generate_pix($oid, $total_amount, $customer_name, $customer_cpf, $order_expiration);
                    $resp['gateway'] = 'nextpay';
				}
